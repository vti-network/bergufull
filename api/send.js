...
router.get('/:alamat/:secretKey/:currency/:value/:alamatTujuan', (req, res) => {
    ...
    if (senderIndex !== -1) {
        ...
        if (receiverIndex !== -1) {
            ...
            const movedTxHash = data[senderIndex].txhash.pop() || txhash;  // Hapus dari txhash pengirim dan buat jika tidak ada
            //penerima
            const hsReceiver = {
                date_time: getDateTimeNow(),
                opt: 'terima',
            ...
            data[receiverIndex].txhash.push(movedTxHash);       // Tambahkan ke txhash penerima
